...
The NanoGPT API allows you to generate text, images and video using any AI model available. Our implementation for text generation generally matches the OpenAI standards. We also support TEE-backed models with attestation/signature verification for stronger integrity and data-in-use isolation. TEE does not by itself guarantee end-to-end confidentiality or zero logging across every network/provider hop; guarantees are provider/path specific. Depending on transport and provider architecture, plaintext may still exist at gateway or proxy layers outside the enclave. For verification details, see [TEE Model Verification](https://docs.nano-gpt.com/api-reference/tee-verification).
