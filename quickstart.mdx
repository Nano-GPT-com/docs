...
Send API requests to `https://api.nano-gpt.com`. The direct API host allows larger uploads and longer-running calls than the website host. See [API Hosts](/api-reference/miscellaneous/api-hosts) for SDK base URLs.
...
<AccordionGroup>
  ...
  <Accordion icon="video" title="Video Generation">
    ...
    curl "https://api.nano-gpt.com/api/video/status?requestId=$RUN_ID" \
      -H "Authorization: Bearer YOUR_API_KEY"
    ```
