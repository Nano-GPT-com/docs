...
description: "Complete guide to text-to-speech synthesis APIs"
---
...
- **Dia-TTS**: Multi-speaker dialogue model with script parsing ($0.05/1k chars)
...
Dia-TTS excels at creating conversations between two speakers:

```python
def create_dialogue(script_parts):
    """
    Create a dialogue using Dia-TTS with speaker tags
    """
    # Format script with speaker tags
    dialogue_text = ""
    for part in script_parts:
        speaker = part["speaker"]  # "S1" or "S2"
        text = part["text"]
...
# Example dialogue
conversation = [
    {"speaker": "S1", "text": "Welcome to our podcast!"},
    {"speaker": "S2", "text": "Thanks for having me on the show."},
    {"speaker": "S1", "text": "Let's start by talking about your background."},
    {"speaker": "S2", "text": "I've been working in AI for over a decade."},
    {"speaker": "S1", "text": "That's fascinating! What got you started?"},
    {"speaker": "S2", "text": "It all began with a computer science course in college."}
]

try:
    response = create_dialogue(conversation)
    print("Dialogue generated successfully!")
except Exception as e:
    print(f"Error: {e}")

# You can also use a single speaker
single_speaker_text = "[S1] This is a single speaker narration using the first voice."
text_to_speech(single_speaker_text, model="Dia-TTS", voice="S2")  # Use S2 voice
```
...
class NanoGPTTTS:
    ...
    def estimate_cost(self, text, model="Kokoro-82m"):
        """Estimate generation cost"""
        model_config = self.model_info.get(model, {})
        cost_per_1k = model_config.get("cost_per_1k", 0.001)
        
        char_count = len(text)
        base_cost = (char_count / 1000) * cost_per_1k
        final_cost = base_cost * 1.1  # 10% profit margin
...
| Dia-TTS | $0.05 | 10,000 | Multi-speaker dialogue |
...
All prices include a 10% profit margin. Minimum charge is $0.001 per request. 