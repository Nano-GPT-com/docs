...
For voice cloning (create a reusable custom voice from a reference clip), see `api-reference/endpoint/voice-cloning.mdx`.
...
Set `stream: true` to request chunked audio streaming. Default remains buffered (`stream: false`).
...
Other TTS models (for example Kokoro, Gemini, Qwen, MiniMax, Inworld) ignore `stream` and return buffered audio.
...
async function submitThenPollTTS({ text, model = 'Elevenlabs-Turbo-V2.5', voice = 'Rachel' }) {
  const submit = await fetch('https://api.nano-gpt.com/api/tts', {
    method: 'POST',
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    const res = await fetch(`https://api.nano-gpt.com/api/tts/status?${qs}`, {
...
curl "https://api.nano-gpt.com/api/tts/status?runId=RUN_ID&model=Elevenlabs-Turbo-V2.5" \
...
class NanoGPTTTS:
    def __init__(self, api_key):
        ...
        self.base_url = "https://api.nano-gpt.com/api"
...
| Model | Cost per 1k chars | Max Length | Streaming (`/v1/audio/speech`) | Special Features |
|-------|-------------------|------------|----------------------------------|------------------|
| Kokoro-82m | $0.001 | 10,000 | No (`stream` ignored) | 44 multilingual voices |
| Elevenlabs-Turbo-V2.5 | $0.06 | 10,000 | Yes | Voice controls, 46 voices |
| Elevenlabs-V3 | Varies | Varies | Yes | Expressive ElevenLabs generation |
| tts-1 | $0.015 | 4,096 | Yes | Multiple formats |
| tts-1-hd | $0.030 | 4,096 | Yes | HD quality, voice instructions |
| gpt-4o-mini-tts | $0.0006 | 4,096 | Yes | Ultra-low cost |
