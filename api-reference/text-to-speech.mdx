...
For voice cloning (create a reusable custom voice from a reference clip), see [Voice Cloning](/api-reference/endpoint/voice-cloning).

## Music generation

NanoGPT also supports text-to-music generation via the OpenAI-compatible `POST /v1/audio/speech` endpoint when you use a music model. See [Music Generation](/api-reference/music-generation).
...
- **Elevenlabs-V3**: Premium quality with natural voices and enhanced controls ($0.10/1k chars)
...
- Synchronous: `POST /v1/audio/speech` returns audio bytes directly. Best for UI playback and short prompts. See the [Speech endpoint](/api-reference/endpoint/speech).
...
| Elevenlabs-V3 | $0.10 | 3,000 | Enhanced controls, natural voices |
