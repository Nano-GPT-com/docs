...
Some endpoints return `402 Payment Required` when your account balance is insufficient. The format depends on whether you've opted into X-402 payment flows.

### Standard Balance Error (without X-X402 header)
...
### X-402 Payment Challenge (with X-X402: true header)

When you include the `X-X402: true` header, insufficient balance triggers the X-402 payment protocol with payment options:
...
See [X-402 Micropayments](/api-reference/miscellaneous/x402) for details on handling payment challenges.
