...
Supported endpoints can also be called without `Authorization` or `x-api-key`. If the request can be quoted, NanoGPT returns a 402 response with a stable top-level `payment` object plus legacy OpenAI-compatible error fields.
...
The legacy `accepts` array may still appear for backwards-compatible and lower-level protocol clients, but new clients should use `payment.accepted[]`. See [Accountless x402 API Payments](/api-reference/miscellaneous/x402) for the complete flow and supported endpoint matrix.
