...
- Native Nano direct deposits: read the linked user's `nanoDepositAddress` from the balance response and show that address to the user. See [Check User Balance](#check-user-balance).
- Crypto invoice or swap deposits: call `POST /api/transaction/create/{ticker}` for supported tickers such as `btc`, `btc-ln`, `ltc`, `ltc-mweb`, `xmr`, `doge`, `dash`, `zec`, `bch`, `kas`, `ton`, `near`, `egld`, `vvv`, `zano`, or `fusd`.
