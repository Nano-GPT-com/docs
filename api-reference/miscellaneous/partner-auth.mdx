...
Partner-authenticated top-ups are not limited to Nano. The same linked NanoGPT account can be funded through the deposit methods NanoGPT supports for that route and deployment, including:
...
- Crypto invoice or swap deposits: call `POST /api/transaction/create/{ticker}`, for example `btc`, `btc-ln`, `ltc`, `zec`, or other supported tickers.
- Daimo multi-chain stablecoin deposits: call `POST /api/transaction/create/daimo/{ticker}` where supported.
- Card deposits: call the card deposit route where enabled for the integration.
