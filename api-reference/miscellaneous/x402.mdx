...
1. Client requests an API resource. For gated endpoints such as `/api/v1/chat/completions`, `/api/tts`, `/api/transcribe`, `/api/v1/embeddings`, and `/api/youtube-transcribe`, include `X-X402: true` on this initial request so the endpoint returns an X-402 challenge instead of the normal auth or balance error.
...
If you are retrying with an `X-PAYMENT` header, that header is enough to continue the payment flow; keeping `X-X402: true` on the retry is also safe.
...
1. Call an API endpoint (no auth). Include `X-X402: true` for endpoints that require explicit opt-in to return an X-402 challenge.
...
```bash
# 1) Request (no auth, opt into X-402 challenge mode)
