...
NanoGPT does not search tools outside the request's authorized catalog. Tool names, descriptions, and schemas should still be treated as untrusted application input.
...
Usage and `x_nanogpt_pricing` continue to follow the normal Responses contract. Search rounds are bounded by the platform, and the request fails rather than entering an unbounded discovery loop.
...
No match is a valid search outcome. The model may search again within platform limits, answer without a function, or return an ordinary incomplete/failed response. Clients should handle cancellation and partial streams using the normal [streaming protocol](/api-reference/miscellaneous/streaming-protocol).
