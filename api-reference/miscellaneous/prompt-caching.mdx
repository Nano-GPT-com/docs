...
description: "Cache large, reusable prompt prefixes for lower latency and cost on supported Claude models, with notes on selected non-Claude caching lanes."
---
...
Explicit cache controls on the routes documented here are configured per request.

## Supported Models and Providers

Explicit `prompt_caching` / `cache_control` controls are available on Claude models, including these families (examples):
...
Several non-Claude routes also support prompt-caching helpers or report cache-usage metadata when the upstream provider exposes it. Current examples include selected OpenRouter Gemini lanes and direct/OpenRouter `qwen-3.6-plus` / SpokeAI routes, but those lanes may expose fewer controls than the Claude routes documented below and can clamp TTL back to `5m`. Treat the TTL and cache-creation pricing guidance on this page as the full Claude-style behavior unless a provider-specific route documents broader support.
...
- Prompt-caching behavior varies by model/provider. Claude offers the most complete explicit controls, while several non-Claude routes (including some Gemini/OpenRouter and selected provider-specific lanes) also support cache helpers or report cache usage. Unsupported models ignore the caching fields.
