...
```http
POST https://api.nano-gpt.com/api/v1/audio/speech
```

## Choosing A Music Model

Music model availability changes over time. Discover available audio models via `GET https://api.nano-gpt.com/api/v1/audio-models` (see [Audio Models](/api-reference/endpoint/audio-models)) and select a model intended for music generation.
...
|-----------|------|----------|-------------|
| `model` | string | Yes | A music-capable audio model ID (discover via `GET https://api.nano-gpt.com/api/v1/audio-models`) |
