...
def chat_completion_with_web_search(messages, model="chatgpt-4o-latest", search_depth="standard"):
    ...
    Args:
        ...
        search_depth: "standard" ($0.005) or "deep" ($0.05)
    """
...
- **`:online`** - Standard search with 10 results ($0.005 per request)
- **`:online/linkup-deep`** - Deep iterative search ($0.05 per request)
