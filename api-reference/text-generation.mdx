...
Pay-as-you-go requests can select an upstream provider for supported open-source models using the `X-Provider` header or saved preferences. Subscription endpoints ignore provider selection unless you force paid routing with the pay-as-you-go billing override (`billing_mode: "paygo"` or `X-Billing-Mode: paygo`). See [Provider Selection](/api-reference/miscellaneous/provider-selection) and [Pay-As-You-Go Billing Override](/api-reference/miscellaneous/billing-override).
...
```python
def chat_completion_with_claude_cache(messages, model="anthropic/claude-opus-4.5"):
    """
...
### Brave Answers & Research

NanoGPT offers three Brave-powered text models that combine web search with LLM generation:

- **`brave`**: Brave (Answers). Performs a single web search and generates a grounded answer. Fast responses (~4.5 seconds), low cost. Good for quick factual questions.
- **`brave-pro`**: Brave (Pro). Premium tier with higher quality answers, more search depth, and priority processing.
- **`brave-research`**: Brave (Research). Performs multiple iterative searches with reasoning and planning for thorough, well-sourced answers. Response times can extend to minutes. Best for complex questions requiring comprehensive research.

All three models use the OpenAI-compatible chat completions format: set `model` to `brave`, `brave-pro`, or `brave-research` and send `messages`.

This is separate from the Web Search integration (`:online/*` model suffixes and the `/api/web` endpoint), which returns raw search results to be injected into another model.

Within the Brave provider for that separate Web Search integration (`:online/brave` or `provider: "brave"`), NanoGPT prefers Brave's LLM Context API when available and falls back to legacy Brave Web Search when needed.

For all Brave request parameters, see:

- [Brave model overview and parameter entry point](/api-reference/endpoint/chat-completion#brave-answers-and-research-models)
- [Brave-specific request fields](/api-reference/endpoint/chat-completion#brave-specific-request-fields)
- [Research tuning parameters](/api-reference/endpoint/chat-completion#research-tuning-parameters)
- [Web search options for Brave models (`web_search_options.search_context_size`, `user_location`)](/api-reference/endpoint/chat-completion#web-search-options-brave-models)

#### Pricing

Brave models use variable pricing based on usage reported upstream (number of searches and total tokens processed). At a high level:
...
- **`brave-pro`**: Similar baseline profile to `brave`, but premium tier behavior may increase effective request cost depending on usage.
- **`brave-research`**: Higher cost due to multiple searches per request; cost scales with query complexity.
