...
Use explicit prompt-caching controls (`prompt_caching`, `promptCaching`, and body-level `cache_control` alias, plus inline `cache_control`) when you need Claude-specific cache boundaries, TTL selection, or `stickyProvider` consistency control.
...
Brave documentation has been moved to a dedicated page: [Brave](/api-reference/miscellaneous/brave).

- Includes Brave Answers model IDs (`brave`, `brave-pro`, `brave-research`)
- Covers all Brave-specific request fields and `web_search_options`
- Includes Brave `/api/web` provider fields, examples, pricing, and terms highlights
