...
### Prompt Caching (Claude + Gemini Models)
...
Prompt caching is available for Claude and Gemini models. Use inline `cache_control` markers on reusable blocks, or use the helper (`prompt_caching`, `promptCaching`, and body-level `cache_control` alias) to auto-mark the leading prefix.
...
```python
def chat_completion_with_prompt_cache(messages, model="google/gemini-3.1-pro-preview"):
    """
