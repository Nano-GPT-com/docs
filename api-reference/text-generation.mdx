...
### Brave Grounding & Research

NanoGPT offers three Brave-powered text models that combine web search with LLM generation:

- **`brave`**: Brave Grounding. Performs a single web search and generates a grounded answer. Fast responses (~4.5 seconds), low cost. Good for quick factual questions.
- **`brave-pro`**: Brave Pro Grounding. Premium tier with higher quality answers, more search depth, and priority processing.
- **`brave-research`**: Brave Deep Research. Performs multiple iterative searches with reasoning and planning for thorough, well-sourced answers. Response times can extend to minutes. Best for complex questions requiring comprehensive research.
...
- [Brave model overview and parameter entry point](/api-reference/endpoint/chat-completion#brave-grounding-and-research-models)
