...
NanoGPT offers two Brave-powered text models that combine web search with LLM generation:
...
Both models use the OpenAI-compatible chat completions format: set `model` to `brave` or `brave-research` and send `messages`.
