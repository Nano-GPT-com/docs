...
Pay-as-you-go requests can select an upstream provider for supported open-source models using the `X-Provider` header or saved preferences. Subscription endpoints ignore provider selection. See [Provider Selection](/api-reference/miscellaneous/provider-selection).
