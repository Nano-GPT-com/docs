...
After paying with one of the returned `payment.accepted[]` options, either call the selected option's `completeUrl` or replay the same request with `X-PAYMENT`, depending on the scheme. See [Accountless x402 API Payments](/api-reference/miscellaneous/x402) and `GET /api/v1/x402/endpoints` for the current endpoint matrix.
