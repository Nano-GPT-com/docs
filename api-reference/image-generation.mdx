...
description: "Guide to the OpenAI-compatible image generation endpoint"
---
...
For authenticated API-key requests, you can opt in to a paid input safety preflight by sending the `moderation` header. See [Inline Moderation](/api-reference/miscellaneous/inline-moderation) for supported image routes, inspectable image inputs, billing behavior, and error codes.
...
`POST /api/v1/images/generations` supports accountless x402 payments. Send the same JSON request without `Authorization` or `x-api-key`; supported deployments return `402 Payment Required` with a machine-readable `payment` quote.
...
This endpoint supports accountless x402 payments where listed by `GET /api/v1/x402/endpoints`, including Lightning L402 when advertised. After paying with one of the returned `payment.accepted[]` options, either call the selected option's `completeUrl`, replay the same request with `X-PAYMENT`, or replay with `Authorization: L402 <token>:<preimage>`, depending on the scheme. See [Accountless x402 API Payments](/api-reference/miscellaneous/x402) for the full flow.
...
If you requested response_format: "url", download the image by performing a normal HTTP GET to data[i].url.
