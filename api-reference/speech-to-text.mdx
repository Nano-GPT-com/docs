...
description: "Complete guide to speech-to-text transcription APIs"
---
...
For drop-in OpenAI SDK compatibility, you can also use the OpenAI-compatible endpoint: `POST /api/v1/audio/transcriptions` (see `api-reference/endpoint/audio-transcriptions.mdx`).

## Available Models

NanoGPT supports multiple Speech-to-Text and audio-to-text workflows, including standard transcription, video transcription, and voice cloning.

| Model ID | Type | Billing | Price |
| --- | --- | --- | --- |
...
Transcription models are billed by audio/video duration. Voice cloning models are billed per run.
...
Costs are calculated based on audio/video duration (transcription) or per run (voice cloning).
