...
| `sort` | string | none | Use `favorites` to order models by the authenticated user's recent model usage |
...
`GET /api/v1/models` supports `sort=favorites`.

When this parameter is present, NanoGPT returns the same model list the caller would normally receive, but ordered by the authenticated user's recent model usage. Models used most often appear first.
...
This can be combined with `detailed=true`:
...
- Sorting is based on completed model usage from the last 90 days.
- The endpoint does not hide unused models. Models with no recent usage remain in the response after the used models, in the normal catalog order.
- The same account, subscription, API key allowlist, and visible-model filters still apply before sorting.
- If no valid API key is provided, or the user has no recent usage, the endpoint falls back to the normal ordering.
