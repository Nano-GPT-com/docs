...
This returns an OpenAI-style list of public speech-to-text models:
...
`transcription` is a discovery selector, not a replacement for existing model metadata. Returned models retain `architecture.modality: "audio->text"` and `architecture.output_modalities: ["text"]`. Voice-cloning tools are excluded.

Repeated and comma-separated `output_modalities` values are parsed and deduplicated. Transcription discovery is activated only when `transcription` is the sole normalized selector. Mixed selectors such as `text,transcription` continue to return the normal text-model catalog.
