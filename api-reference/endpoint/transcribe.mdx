...
Looking for a drop-in OpenAI-compatible STT endpoint? Use `POST /api/v1/audio/transcriptions`. See [Create an audio transcription](/api-reference/endpoint/audio-transcriptions).
...
The complete multipart or JSON request body must not exceed **4 MiB**. Multipart boundaries, form fields, the JSON envelope, and base64 overhead count toward the limit.

For multipart uploads, send the file as `audio` or `file`.
...
Use a remotely reachable URL for larger media. The maximum remote file size varies by model; do not assume a universal 500 MB limit.

Multipart and JSON requests accept `file_url`, `audio_url`, or `audioUrl`. In JSON, these aliases can also contain a supported base64 audio data URI, such as `data:audio/wav;base64,BASE64_AUDIO_BYTES`. The complete JSON body, including base64 expansion, must fit within 4 MiB.
