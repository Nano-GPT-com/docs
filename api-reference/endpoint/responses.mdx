...
| `seed` | integer | No | Optional integer forwarded to providers that support seeded sampling. This may improve reproducibility but does not guarantee identical output. Results can change if NanoGPT selects a different route, uses a fallback, or the provider changes its backend. |
