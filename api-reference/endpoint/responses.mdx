...
| --- | --- | --- |
| `effort` | See below | Reasoning depth. Any value other than `none` explicitly requests reasoning mode. |
...
Possible values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`. The exact accepted subset depends on the selected model. Check `reasoning_efforts` from `GET /api/v1/models?detailed=true`.
