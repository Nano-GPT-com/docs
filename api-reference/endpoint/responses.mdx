...
For supported accountless x402 requests, omit `Authorization` and `x-api-key` to receive a payment quote. The advertised schemes, including Lightning L402 when enabled, are listed by `GET /api/v1/x402/endpoints`.
