...
| `store` | boolean | No | Store response for later retrieval. Synchronous responses persist only when this is explicitly set to `true`; background requests still require persisted state and reject `store: false` |
| `retention_days` | integer or null | No | Override stored-response retention for this request (`0-365`, `0` disables retention, `null` uses the account/team default) |
| `retentionDays` | integer or null | No | Camel-case alias for `retention_days` |
...
- Requires effective retention greater than `0`
