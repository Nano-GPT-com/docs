...
- **Midjourney extend (task-based)**: use `POST /api/generate-video/extend` with `taskId` and `index` (0-3). This flow does not accept `video`, `videoUrl`, `videoDataUrl`, or `videoAttachmentId`.
...
- Poll `/api/generate-video/status?runId=<runId>&model=<model>` until the job reaches `status: "COMPLETED"` or `status: "FAILED"`.
