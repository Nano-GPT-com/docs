...
Supported deployments return `402 Payment Required` with a top-level `payment` quote when the request is unauthenticated and can be quoted. See [Accountless x402 API Payments](/api-reference/miscellaneous/x402) and `GET /api/v1/x402/endpoints` for the current endpoint matrix.
