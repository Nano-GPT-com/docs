...
The standalone Context Memory endpoint compresses an entire conversation into a single memory message using Polychat's Context Memory. This endpoint does not run a model. It returns the compressed memory message and usage so you can pipe it into your own chat completion request or store it.

- No model inference is performed
- Pass your `messages` array and optional settings
- We return `{ messages, usage }` from Polychat and bill exactly their cost (no markup)

See the feature overview in `api-reference/miscellaneous/context-memory`.
...
Standard abuse protection and rate limits apply.
...
- `usage`: Polychat token usage. When available, `prompt_tokens_details.cached_tokens` indicates discounted cached input tokens
...
- Exact pass-through of Polychat costs; no markup
- Non-cached input tokens: $5.00 / 1M
- Cached input tokens: $2.50 / 1M (when applicable)
- Output tokens: $10.00 / 1M
- Balance precheck performed using estimated input tokens at $5.00 / 1M
...
- Use this endpoint to compress long histories, then call `POST /v1/chat/completions` with the returned `messages`
- Alternatively, enable memory directly in chat completions by appending `:memory` to the model or sending the `memory: true` header

For the full feature overview, see `api-reference/miscellaneous/context-memory`. 