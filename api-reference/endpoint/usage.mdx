...
For one recent request, use [Request Billing](/api-reference/endpoint/request-billing)
with its captured `X-Request-ID` and the same inference API key. That lookup has
