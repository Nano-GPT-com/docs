...
Looking for synchronous, low‑latency TTS that returns audio bytes directly? See `api-reference/endpoint/speech.mdx` (POST `/v1/audio/speech`).
...
For OpenAI-compatible music generation via `POST /api/v1/audio/speech`, see `api-reference/music-generation.mdx`.
