...
When you use a music model (for example `Minimax-Music-02`), the `input` field is treated as a music prompt (not text to speak) and `voice` is ignored. For a dedicated music guide and model list, see [Music Generation](/api-reference/music-generation).
...
- `stream` (boolean, optional): When `true`, returns chunked audio bytes progressively on supported models/providers.
...
- Streaming (`stream: true` on supported models/providers): audio bytes arrive progressively, usually without `Content-Length`, so clients can start playback before generation finishes.
...
- Streaming support is currently exposed on OpenAI TTS models and the ElevenLabs Turbo/V3 lanes; other models return buffered audio.
...
- Voice IDs vary by model/provider. See model-specific voices on the [Text-to-Speech overview](/api-reference/text-to-speech).
...
- Rate limits: per-minute/day caps; contact support to request increases. See [Pricing](/api-reference/miscellaneous/pricing) and [Rate Limits](/api-reference/miscellaneous/rate-limits).
...
- [Async/job-based TTS](/api-reference/endpoint/tts)
- [TTS Status polling](/api-reference/endpoint/tts-status)
