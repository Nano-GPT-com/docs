---
title: 'v1/speech (Synchronous TTS)'
...
- Method/Path: `POST https://nano-gpt.com/api/v1/speech`
...
curl -X POST \
  ...
  -H "Accept: audio/mpeg" \
  https://nano-gpt.com/api/v1/speech \
  -d '{
    "model": "nano-tts-1",
...
const res = await fetch("https://nano-gpt.com/api/v1/speech", {
  method: "POST",
  headers: {
...
```ts
const res = await fetch("https://nano-gpt.com/api/v1/speech", { /* headers/body as above */ });
if (!res.ok) throw new Error(await res.text());
