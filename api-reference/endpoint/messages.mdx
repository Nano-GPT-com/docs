...
<Note>
  TypeSafe Jev decision models use `output_config.format: {"type":"questions","questions":{...}}`, accept only non-streaming user text, and return the answer object as JSON text in `content[0].text`. See [Jev Decisions](/api-reference/endpoint/jev-decisions) for complete examples and limitations.
</Note>
