...
<Note>
  To set reasoning effort through a custom model ID, append `:reasoning-effort/high` (also `low`, `medium`, `xhigh`, or `max`). NanoGPT strips the suffix before routing and uses it as the default `output_config.effort`. Explicit body generation settings take precedence; supported efforts remain model/provider-specific. See [Reasoning Effort Suffixes](/api-reference/miscellaneous/model-suffixes#reasoning-effort-suffixes).
</Note>

import { exampleTextModel, exampleTextModelName } from '/snippets/example-text-model.mdx';

<Note>
  **API host:** Use `https://api.nano-gpt.com/api` as the base URL for the Anthropic SDK, which appends `/v1/messages`. For raw HTTP requests, use `https://api.nano-gpt.com/api/v1/messages`. See [API Hosts](/api-reference/miscellaneous/api-hosts).
</Note>
...
<Note>
  TypeSafe Jev decision models use `output_config.format: {"type":"questions","questions":{...}}`, accept only non-streaming user text, and return the answer object as JSON text in `content[0].text`. Jev currently requires the website host. See [Jev Decisions](/api-reference/endpoint/jev-decisions) for complete examples and limitations.
</Note>
...
```
POST https://api.nano-gpt.com/api/v1/messages
```
...
```bash
curl -X POST https://api.nano-gpt.com/api/v1/messages \
...
```bash
curl -N -X POST https://api.nano-gpt.com/api/v1/messages \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "model": "claude-opus-4-5-20251101",
...
1. Change the base URL:
   ...
   The full endpoint will be: `https://api.nano-gpt.com/api/v1/messages`
