...
<AccordionGroup>
  ...
  <Accordion title="Images & caching">
    ...
    - [Prompt Caching (Claude + Gemini Models)](#prompt-caching-claude-gemini-models)
...
Prompt caching is available for Claude and Gemini models. You can either place inline `cache_control` blocks on reusable content, or use the body-level helper (`prompt_caching`, `promptCaching`, and `cache_control` alias).
...
- Gemini and Claude both support `5m` and `1h` TTLs, with provider-specific pricing. See [Prompt Caching](/api-reference/miscellaneous/prompt-caching#pricing).
