...
<AccordionGroup>
  ...
  <Accordion title="Web search">
    ...
    - [Brave reference (dedicated page)](/api-reference/miscellaneous/brave)
...
### Brave reference (dedicated page)

Brave documentation has been moved to a dedicated page: [Brave](/api-reference/miscellaneous/brave).

- Use `:online/brave` or `webSearch.provider = "brave"` for raw web results injected into another model.
- Use `model: "brave"`, `model: "brave-pro"`, or `model: "brave-research"` for Brave Answers models.
- For Brave-specific parameters (`enable_research`, research tuning fields, `web_search_options`, and examples), see the dedicated Brave reference.
...
Brave-specific terms highlights are documented on [Brave](/api-reference/miscellaneous/brave).
...
`reasoning_effort` controls reasoning depth and also acts as an explicit reasoning-mode signal.
Any value other than `"none"` is treated as a request to enable reasoning/thinking behavior.
Use `"none"` to explicitly disable reasoning.
