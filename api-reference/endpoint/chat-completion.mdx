...
<Note>
  Provider selection is available for pay-as-you-go requests on supported open-source models. Set the `X-Provider` header or save preferences to choose a provider. See [Provider Selection](/api-reference/miscellaneous/provider-selection).
</Note>
...
OpenAI native web search: GPT-5 (all versions) and the o1/o3/o4 series use OpenAI's built-in web search automatically. No suffix is required; you can still set `webSearch.search_context_size` and `webSearch.user_location`. To force a different provider, specify a provider or suffix.
...
- `provider` (string): `linkup` | `tavily` | `exa` | `kagi` | `perplexity` | `valyu`
- `depth` (string):
  - Linkup/Tavily/Perplexity/Valyu: `standard` or `deep`
...
##### OpenAI native (GPT-5 and o1/o3/o4 series)
...
BYOK is supported for Tavily, Exa, Kagi, Perplexity, and Valyu. Linkup always uses NanoGPT's key. When you supply your own provider key, NanoGPT does not add a platform charge for web search. Configure keys in team settings or pass them via headers.
