...
| `max` | Maximum reasoning depth, supported only by models that advertise it |

These are the possible values across NanoGPT, not a guarantee that every model supports all of them. Supported values vary by model. Query `GET /api/v1/models?detailed=true` and inspect the model's `reasoning_efforts` field before sending a value.

Unsupported values are rejected with `unsupported_reasoning_effort`, and the error response includes the model's supported values.
