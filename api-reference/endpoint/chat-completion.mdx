...
|---|---|---|---|
| `research_allow_thinking` | boolean | — | Allow the research model to show its thinking/reasoning process in the response. |
...
| `web_search_options.search_context_size` | string | Amount of search context: `"low"`, `"medium"`, or `"high"`. |
...
| `web_search_options.user_location.approximate` | object | Contains `city`, `country`, `region`, and `timezone` (nullable strings). |
