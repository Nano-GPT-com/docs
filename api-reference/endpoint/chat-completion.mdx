...
<AccordionGroup>
  ...
  <Accordion title="Web search">
    ...
    - [Brave Grounding and Research models](#brave-grounding-and-research-models)
...
<Note>
  ...
  2) `model: "brave"` / `model: "brave-pro"` / `model: "brave-research"` uses Brave's own grounded answer pipeline (search + generation in one model).
</Note>
...
### Brave Grounding and Research models

NanoGPT also exposes two Brave-powered text models that combine web search with generation. Use them by setting the `model` directly:

| Model ID | Description |
|---|---|
| `brave` | Brave Grounding. Single search + answer. Fast, low cost. |
...
When using `model: "brave"` or `model: "brave-research"`, you can pass these optional top-level fields in the request body (OpenAI SDK `extra_body` style):

| Parameter | Type | Default (`brave`/`brave-pro`) | Default (`brave-research`) | Description |
|---|---|---|---|---|
| `enable_research` | boolean | `false` | `true` | Enables multi-search deep research mode with reasoning. Performs multiple iterative searches for a thorough answer. Requires streaming. |
...
| `research_allow_thinking` | boolean | — | — | Allow the research model to show its thinking/reasoning process. |
| `research_maximum_number_of_iterations` | integer | — | — | Maximum number of research iterations. |
| `number_of_queries` | integer | — | — | Maximum number of search queries to issue during research (1-50). |
...
Example (override defaults on `brave`):
