...
<Note>
  TypeSafe Jev decision models use `response_format: {"type":"questions","questions":{...}}`, accept only non-streaming user text, and return the answer object as JSON text in `choices[0].message.content`. See [Jev Decisions](/api-reference/endpoint/jev-decisions) for complete examples and limitations.
</Note>
