...
NanoGPT supports OpenAI-compatible structured outputs using the `response_format` parameter. This ensures the model's response matches a specific JSON schema.

#### Request parameters

- `response_format` (optional object): Set to `{ "type": "json_schema", "json_schema": { "name": string, "strict": boolean, "schema": object } }` to enforce a JSON schema. `strict: true` is recommended for guaranteed adherence.
...
```json
{
  ...
  "messages": [
    ...
    { "role": "user", "content": "Alice is 25 and Bob is 30." }
  ],
  "response_format": {
    "type": "json_schema",
    "json_schema": {
...
Send a `linkup` object in the request body. This works with or without a model suffix and controls web search across all providers.
...
// Request-body configuration (Exa neural)
const linkupResponse = await fetch(`${BASE_URL}/chat/completions`, {
    method: 'POST',
    headers: {
        'Authorization': `Bearer ${API_KEY}`,
        'Content-Type': 'application/json'
