...
<AccordionGroup>
  ...
  <Accordion title="Sampling & decoding">
    ...
    - [Logit Shaping & Seed Reproducibility](#logit-shaping-seed-reproducibility)
...
### Logit Shaping & Seed Reproducibility
...
| `seed` | integer | Optional integer forwarded to providers that support seeded sampling. This may improve reproducibility but does not guarantee identical output. Results can change if NanoGPT selects a different route, uses a fallback, or the provider changes its backend. |
...
Seeded generation is best-effort and is not a byte-identical output guarantee. To reduce variability across requests:

- Keep the exact model, messages, tools, and sampling settings unchanged.
- Select a specific provider where provider selection is supported.
- Disable automatic fallbacks where routing controls expose that option.
- Use a low or zero `temperature` where the selected model and provider support it.
- Record provider and system-fingerprint metadata if NanoGPT exposes stable, reliable values for the route; do not treat metadata as a guarantee by itself.

NanoGPT will publish a seed-support matrix when route-level capability data is reliable enough to maintain one. Until then, treat `seed` as a best-effort hint and use [Provider Selection](/api-reference/miscellaneous/provider-selection) to reduce routing variability where supported.
