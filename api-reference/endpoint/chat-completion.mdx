...
<Note>
  ...
  2) `model: "brave"` / `model: "brave-research"` uses Brave's own grounded answer pipeline (search + generation in one model).
</Note>
...
| Parameter | Type | Default (`brave`) | Default (`brave-research`) | Description |
|---|---|---|---|---|
...
**Streaming requirement:** `enable_research`, `enable_citations`, and `enable_entities` require `stream: true`. Since `brave-research` defaults to `enable_research: true`, `brave-research` requires `stream: true`.
