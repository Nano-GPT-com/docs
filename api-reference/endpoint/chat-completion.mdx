...
<Note>
  Brave also offers an LLM Context API (search-derived, relevance-ranked chunks for LLM workflows). NanoGPT's `:online/brave` / `webSearch.provider = "brave"` integration currently prefers LLM Context when available and falls back to legacy Brave Web Search when needed; the Brave Answers model family above is a separate integration.
</Note>
...
| `enable_citations` | boolean | `false` | `false` | Includes `<citation>` markup in the response text linking to sources. Requires streaming, and is forced to `false` whenever research mode is enabled. |
| `enable_entities` | boolean | `false` | `false` | Includes entity metadata in the response. Requires streaming, and is forced to `false` whenever research mode is enabled. |
...
NanoGPT automatically applies implicit caching on providers/models that support it, so most requests do not need caching flags. Use explicit prompt-caching controls (`prompt_caching`, `promptCaching`, and inline `cache_control`) when you need Claude-style cache boundaries, TTL selection, or `stickyProvider` consistency control.
...
- Inline `cache_control` requests should include the upstream `anthropic-beta` prompt-caching header. If you use the helper-based `prompt_caching` fields on `/v1/chat/completions`, NanoGPT adds it automatically.
