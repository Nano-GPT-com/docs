...
<Note>
  Brave-specific documentation now lives on a dedicated page: [Brave](/api-reference/miscellaneous/brave). This page's `provider: "brave"` mode returns raw search results; Brave Answers models (`model: "brave"`, `model: "brave-pro"`, `model: "brave-research"`) are documented there.
</Note>
...
Brave provider options (`count`, `maxResults`, `offset`, `safesearch`, `freshness`, `searchLang`, `uiLang`, `eSnippets`, `spellcheck`, `goggles`, `country`) are documented on [Brave](/api-reference/miscellaneous/brave).
...
For a Brave-specific request example, see [Brave](/api-reference/miscellaneous/brave).
...
7. **Respect provider data policies**: Cache only where provider terms allow it.
