...
<Note>
  The Brave provider on this page (`provider: "brave"` on `/api/web`, or `:online/brave` on chat completions) uses NanoGPT's Brave web-search integration. It prefers Brave's LLM Context API and automatically falls back to legacy Brave Web Search when the key lacks LLM Context entitlement or when you send legacy-only filters.
</Note>

<Note>
  This is different from the Brave Answers text models (`model: "brave"`, `model: "brave-pro"`, and `model: "brave-research"`), which generate complete answers using Brave's own search + LLM pipeline.
</Note>
...
<ParamField body="kagiSource" type="string">
  Kagi only. Source to query: "web", "news", or "search"
</ParamField>
...
Exa crawl and subpage extraction controls must be nested under `contents`.
...
Use either `count` or `maxResults`. When Brave LLM Context is available, `/api/web` currently returns grounded content for up to 20 URLs on standard searches and up to 30 URLs on deep searches, even though `count`/`maxResults` accepts values up to 50. If the key lacks that entitlement, or if you send legacy-only filters (`offset`, `freshness`, non-default `safesearch`, `uiLang`, `extraSnippets`, or `spellcheck`), NanoGPT automatically falls back to Brave Web Search, where the effective result cap is 20. `searchLang` and `uiLang` are passed to Brave as `search_lang` and `ui_lang`. If `freshness` is not provided but both `fromDate` and `toDate` are set, the API builds a Brave freshness range (`fromDate + "to" + toDate`). For deep searches on the legacy path, Brave defaults `extraSnippets` to `true` unless you set it explicitly.
...
if results["metadata"]["outputType"] == "searchResults":
    for result in results["data"]:
        ...
        print(f"Snippet: {result.get('content', 'N/A')[:200]}...")
        print("-" * 50)
...
# Search results with domain filtering
curl -X POST https://nano-gpt.com/api/web \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
...
{
  "error": "Insufficient balance. A minimum balance of $0.10 (USD equivalent) is required to use web search."
}
```
...
7. **Respect provider data policies**: Cache only where provider terms allow it. Brave restricts caching to transient storage.
...
| Exa | $0.005 base | $0.012 base (+ $0.001/page contents) | Neural: $0.010 base, deep-reasoning: $0.015 base |
...
A minimum balance of $0.10 (USD equivalent) applies to billed `/api/web` searches. BYOK requests skip this balance check. OpenAI native applies to GPT-5/o-series chat completions only and is not used by `/api/web`.

## Bring your own key (BYOK)

BYOK availability is provider-dependent and can change over time. See the BYOK reference for the current support matrix.
