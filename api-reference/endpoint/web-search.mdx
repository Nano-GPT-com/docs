...
<Note>
  The Brave provider on this page (`provider: "brave"` on `/api/web`, or `:online/brave` on chat completions) returns raw search results that you can inject into another model. This is different from the Brave grounding text models (`model: "brave"` and `model: "brave-research"`), which generate complete answers using Brave's own search + LLM pipeline.
</Note>
