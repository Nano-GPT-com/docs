...
<Note>
  The Brave provider on this page (`provider: "brave"` on `/api/web`, or `:online/brave` on chat completions) returns raw search results that you can inject into another model. This is different from the Brave Answers text models (`model: "brave"`, `model: "brave-pro"`, and `model: "brave-research"`), which generate complete answers using Brave's own search + LLM pipeline.
</Note>

<Note>
  Brave also offers an LLM Context API (pre-processed, relevance-ranked chunks in LLM-friendly formats). NanoGPT does not currently expose that Brave API surface on `/api/web`.
</Note>
