...
openapi: 'GET /personalized/v1/models'
---
...
curl -H "Authorization: Bearer $NANOGPT_API_KEY" \
  https://api.nano-gpt.com/api/personalized/v1/models
```
...
curl -H "x-api-key: $NANOGPT_API_KEY" \
  "https://api.nano-gpt.com/api/personalized/v1/models?detailed=true"
```
...
{
  "visibleTextModels": {
    "{{example-text-model}}": true,
    "anthropic/claude-opus-4.5": false
  },
  "modelCategories": {
    "{{example-text-model}}": "General"
  }
}
```
