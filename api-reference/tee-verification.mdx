...
```bash
curl "https://api.nano-gpt.com/api/v1/tee/attestation?model=TEE/llama-3.3-70b-instruct" \
...
```bash
curl "https://api.nano-gpt.com/api/v1/tee/signature/{requestId}?model=TEE/llama-3.3-70b-instruct&signing_algo=ecdsa" \
  -H "Authorization: Bearer $API_KEY"
```
