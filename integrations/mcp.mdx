...
| `nanogpt_image_generate` | Generate images using GPT Image 2.5 Flare and other supported models |
...
| `NANOGPT_DEFAULT_MODEL` | Package-dependent | Default model for chat requests; set to `anthropic/claude-sonnet-5` for our recommended configuration |
...
Set `NANOGPT_DEFAULT_MODEL=anthropic/claude-sonnet-5` in your MCP client configuration to use Claude Sonnet 5 for chat requests that omit `model`. Installed package versions can have older built-in defaults.
...
- `model` (optional): Model ID (uses `NANOGPT_DEFAULT_MODEL` when omitted)
...
- `model` (optional): Image model to use (e.g., "openai/gpt-image-2.5/flare/text-to-image")
