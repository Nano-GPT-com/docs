...
On native Windows, `npx` is usually a `.cmd` shim. Launching it through `cmd /c` avoids a bad MCP setup that can look like an API key problem.
...
On native Windows, replace the trailing `-- npx -y @nanogpt/mcp` with `-- cmd /c "C:\Program Files\nodejs\npx.cmd" -y @nanogpt/mcp`.
