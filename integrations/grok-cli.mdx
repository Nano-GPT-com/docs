...
<Steps>
  ...
  <Step title="Set Required Environment Variables">
    ...
    |----------|-------|-------------|
    | `GROK_BASE_URL` | `https://api.nano-gpt.com/api/v1` | NanoGPT OpenAI-compatible endpoint |
    | `GROK_API_KEY` | `sk-nano-YOUR-API-KEY` | Your NanoGPT API key |

    <Tabs>
      <Tab title="macOS & Linux">
        ```bash
        export GROK_BASE_URL="https://api.nano-gpt.com/api/v1"
      ...
      <Tab title="Windows PowerShell">
        ```powershell
        $env:GROK_BASE_URL = "https://api.nano-gpt.com/api/v1"
      ...
      <Tab title="Windows Cmd">
        ```cmd
        set GROK_BASE_URL=https://api.nano-gpt.com/api/v1
  ...
  <Step title="Optional: Make It Persistent">
    ...
    ```bash
    echo 'export GROK_BASE_URL="https://api.nano-gpt.com/api/v1"' >> ~/.zshrc
...
grok --model {{example-text-model}}
grok --model anthropic/claude-opus-5.5
grok --model google/gemini-flash-latest
```
